...
import { Sparkles, FileText, BarChart3, Lightbulb, Check, Copy, ChevronDown, ChevronUp, Image } from 'lucide-react'
...
export default function AIGenerator() {
  ...
  function handleGenerate() {
    ...
    const slides = generateCarouselSlides({ mode, input: input.trim(), slideCount, isSingleAd })
    dispatch({ type: 'GENERATE_SLIDES', payload: slides })
    if (isSingleAd) {
      dispatch({ type: 'SET_NAME', payload: input.trim().substring(0, 40) })
    }
  ...
  return (
    <div className="p-4 space-y-4" style={{ maxWidth: '100%', overflow: 'hidden', boxSizing: 'border-box' }}>
      ...
      <div className="flex items-center justify-between">
        <Label className="text-xs font-semibold">Tipo de criativo</Label>
        <div className="flex items-center gap-2">
          <span className={cn("text-xs", !isSingleAd ? "text-foreground font-medium" : "text-muted-foreground")}>Carrossel</span>
          <Switch checked={isSingleAd} onCheckedChange={setIsSingleAd} />
          <span className={cn("text-xs", isSingleAd ? "text-foreground font-medium" : "text-muted-foreground")}>Ad único</span>
        </div>
      </div>
      ...
      <div className="space-y-2">
        <Label className="text-xs">
          {isSingleAd ? 'Texto do anúncio' : mode === 'script' ? 'Cole seu roteiro / texto' : 'Tema do carrossel'}
        </Label>
        {isSingleAd || mode === 'topic' ? (
          <Input value={input} onChange={e => setInput(e.target.value)} placeholder={isSingleAd ? "Ex: Oferta especial - 50% OFF hoje" : "Ex: 5 Dicas de Produtividade"} className="h-9 text-sm" style={{ maxWidth: '100%' }} />
        ) : (
      ...
      <Button className="w-full gap-1.5" onClick={handleGenerate} disabled={!input.trim()} style={{ maxWidth: '100%' }}>
        {isSingleAd ? <Image className="w-4 h-4 shrink-0" /> : <Sparkles className="w-4 h-4 shrink-0" />}
        {isSingleAd ? 'Gerar Anúncio' : 'Gerar Carrossel'}
      </Button>
                  ...
                  <div key={globalIdx} className={cn('p-2.5 rounded-lg border text-xs space-y-1.5', isApplied ? 'border-green-500/30 bg-green-500/5' : 'border-border bg-accent/30')} style={{ overflow: 'hidden', maxWidth: '100%' }}>
                    ...
                    {s.action && (
                      <div className="flex items-center gap-1.5 pt-1" style={{ maxWidth: '100%' }}>
                        <div style={{ flex: 1, minWidth: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }} className="px-2 py-1 rounded bg-background border border-border text-[10px] font-mono text-muted-foreground">{s.action.value}</div>
                        <button onClick={() => applySuggestion(globalIdx)} disabled={isApplied} style={{ flexShrink: 0 }} className={cn('px-2 py-1 rounded text-[10px] font-medium transition-colors', isApplied ? 'bg-green-500/20 text-green-500' : 'bg-primary text-primary-foreground hover:bg-primary/90')}>{isApplied ? 'OK' : 'Aplicar'}</button>
                      </div>
                    )}
                    {s.copyText && (
                      <div className="space-y-1.5 pt-1">
                        <div className="px-2 py-1.5 rounded bg-background border border-border text-[10px] font-mono text-muted-foreground leading-relaxed" style={{ wordBreak: 'break-word', overflowWrap: 'anywhere', maxHeight: '120px', overflow: 'auto' }}>{s.copyText}</div>
                        <button onClick={() => copyPrompt(globalIdx, s.copyText!)} className={cn('flex items-center gap-1 px-2 py-1 rounded text-[10px] font-medium transition-colors', isCopied ? 'bg-green-500/20 text-green-500' : 'bg-accent text-foreground hover:bg-accent/80')}>
